...
Records wall-clock, per-hit times, GPU VRAM, and host RAM / process RSS.
...
def _read_proc_status() -> dict:
    """Process memory from /proc/self/status (Linux/WSL). Values in MiB."""
    out = {}
    try:
...
def _read_meminfo() -> dict:
    """Host RAM from /proc/meminfo. Values in MiB."""
            ...
            for line in fh:
                for k in keys:
                    if line.startswith(k + ":"):
                        raw[k] = float(line.split()[1]) / 1024.0  # kB -> MiB
    except Exception as exc:
...
def memory_snapshot(label: str, *, with_gpu: bool = True) -> dict:
    """Host RAM + process RSS/VMS (+ optional GPU VRAM) at a named checkpoint."""
    host = _read_meminfo()
    proc = _read_proc_status()
    # resource.getrusage maxrss is kB on Linux
    try:
    ...
    snap = {
        ...
        **host,
        ...
        "proc_hwm_mib": round(proc.get("VmHWM", 0.0), 1),  # peak RSS so far
...
def main() -> int:
    ...
    out_name = os.environ.get("AGF_PROFILE_OUT", "profile_g0_17")
    ...
    if not report["torch"]["cuda_available"]:
        print("ERROR: CUDA not available — refusing CPU fallback profile", flush=True)
    ...
    for i, h in enumerate(hits, 1):
        ...
        try:
            ...
            # RAM every hit (GPU every hit too — cheap enough at ~17 samples)
            ...
            for line in fh:
                ...
                if line:
                    try:
                        ...
                        pass
        report["diag_n_lines"] = len(diags)
        report["diag_sample_keys"] = sorted(diags[0].keys()) if diags else []
...
def _print_tables(report: dict) -> None:
    ...
    print("=" * 78)
    print(
        f"{'t_s':>8} {'label':32} {'RSS':>8} {'HWM':>8} {'host_used':>10} {'GPU':>8}"
    )
    print("-" * 78)
    for m in report["memory_samples"]:
        gpu = m["gpu"].get("mem_used_mib") if isinstance(m.get("gpu"), dict) else None
        print(
            f"{m['t_s']:8.1f} {m['label']:32} {m['proc_rss_mib']:8.0f} "
            f"{m['proc_hwm_mib']:8.0f} {m.get('host_used_mib') or 0:10.0f} "
            f"{(gpu or 0):8.0f}"
        )
