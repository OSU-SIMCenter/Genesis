            ...
            try:
                with h5py.File(shard_path, 'r', swmr=True) as h5f:
                    ...
                    for ep_meta in episodes:
                        ...
                        particles_temp_flat = ep_group['observations/state/particles/temp_values'][:]
                        ...
                        for t in range(num_frames):
                            ...
                            temp_cloud_t = particles_temp_flat[start_idx:end_idx]
...
def ragged_collate_fn(batch):
    ...
    for item in batch:
        ...
        # Pad particles and temperatures with zeros to max size
        pad_size = max_particles - num_pts
        if pad_size > 0:
            padded_pts = np.pad(pts, ((0, pad_size), (0, 0)), mode='constant')
            padded_temps = np.pad(item['particles_temp'], (0, pad_size), mode='constant')
        else:
            padded_pts = pts
            padded_temps = item['particles_temp']
        ...
        batch_particles_temp.append(padded_temps)
    ...
    return {
        ...
        'particles_temp': torch.tensor(np.array(batch_particles_temp), dtype=torch.float32),
