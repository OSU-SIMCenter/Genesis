# Surface Reconstruction Research Report: Solid Rigid-Body Simulation
...
This document is intended for an external AI model to evaluate our current pipeline, assess our approach for fighting fluid-like SPH artifacts, and recommend alternative implementations if they can satisfy the performance constraints.

---

## 1. The Performance Constraint (Real-Time / 30 FPS)
A massive secondary goal underlying every decision is **Performance**. 

Since this reconstruction pipeline is running live during a real-time teleoperation simulation loop, the *entire* pipeline (extracting particles -> density splatting -> blurring -> meshing -> temporal smoothing) must comfortably execute in **under 33 milliseconds per frame** to maintain a 30 FPS target.

Without this constraint, we could simply use millions of particles with massive grid resolutions (e.g. $512^3$) running CPU-based algorithms like SplashSurf to achieve perfect visual fidelity. However, those approaches take upwards of 120ms to 500ms per frame. 

Therefore, every optimization we implemented is a careful trade-off: attempting to maximize the visual smoothness of the "rigid" metal while drastically minimizing the computational cost using GPU parallelization, caching, and algorithmic shortcuts. Our current implementation averages ~27.8ms per frame for a 10k particle mesh.
...
1.  **The "Bumpy Grapes" Texture:** When particles compress during a strike, isotropic (spherical) kernels cause the surface to bulge outwards at particle centers, making the smooth metal look like a cluster of grapes.
2.  **Temporal Shimmering (Rippling):** Marching Cubes extracts vertices at precise points along the grid based on the density gradient. Even at rest, tiny floating-point fluctuations in particle positions cause the resulting mesh vertices to constantly "shimmer" or ripple across the surface like water.
3.  **Contour Banding (Staircasing):** If the Marching Cubes threshold was set incorrectly, the mesh would clearly show the discrete 3D grid voxel layers, making the cylinder look like it was built out of Lego bricks or topographical contour lines.
4.  **CPU Bottlenecks:** Original implementations (like `SplashSurf` or SciPy Gaussian Blurs) took 80ms to 120ms to process a 10,000 particle mesh on the CPU, making real-time teleoperation impossible.

---

## 3. What We Have Implemented (Our Pipeline)
To solve these issues and achieve our <33ms target, we built a **Hybrid GPU-to-CPU Pipeline**:

1.  **Taichi GPU Density Splatting:** We abandoned CPU-based neighborhood searches (SplashSurf). Instead, we use Taichi to splat particles into a $128^3$ voxel density grid entirely on the GPU. This eliminates the $O(N)$ CPU bottleneck.
2.  **Mathematical SPH Grounding:** We stopped guessing parameters. We dynamically derive the grid cell size (`dx`) and the particle influence radius from the MPM engine's baseline `particle_radius` (e.g., `influence_radius = 3.0 * radius`).
3.  **Wendland C2 Kernel:** We replaced the standard `(1-r²)^3` isotropic kernel with the Wendland C2 kernel `(1-r)⁴(4r+1)`. This flatter kernel smooths out the "bumpy grapes" texture under compression without requiring computationally expensive higher grid resolutions.
4.  **Temporal Density Blending (Grid-Level):** We maintain a history of the density grid. We blend the current frame's density into the previous frame's density using an exponential moving average. We use an adaptive alpha (0.8 during deformation, 0.2 at rest) to heavily smooth out shimmering when the metal cylinder is still.
5.  **Separable GPU Gaussian Blur:** Before the grid leaves the GPU, we run a 3-pass separable Gaussian blur directly on the density field in Taichi. This mathematically forces the discrete particle splats to "melt" together into a continuous solid mass.
6.  **Fixed Grid Snapping:** We lock the origin of our 3D grid to multiples of `dx`. If the grid origin floats freely with the particles, the Marching Cubes vertices shift wildly. Locking the grid freezes the surface topology in place.
7.  **Dynamic Thresholding:** Instead of a hard-coded isolevel of `0.5`, we dynamically calculate the threshold as `0.4 * max_density`. This ensures the Marching Cubes algorithm always slices the smoothest, most linear section of the density gradient, destroying "Lego" banding.
8.  **PyVista Flying Edges:** We transfer the final blurred density grid from the GPU to the CPU and feed it into `pyvista.ImageData.contour(method='flying_edges')`. This is effectively a highly optimized C++ Marching Cubes implementation that extracts the 3D mesh 2-5x faster than generic libraries.
9.  **Vertex Correspondence Blending (Mesh-Level):** After PyVista extracts the vertices, we use a `scipy.spatial.cKDTree` to find the nearest vertex from the *previous* frame. We blend the new vertex 15% towards the old vertex. This acts as a spatial low-pass filter, locking the vertices in 3D space and eradicating any final high-frequency surface jitter.
10. **Taubin Volume-Preserving Smoothing:** As a final pass, we run 3 iterations of Taubin smoothing (`trimesh.smoothing.filter_taubin`) on the mesh, which smooths out sharp edges without shrinking the overall volume of the cylinder.
