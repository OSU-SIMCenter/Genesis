...
def test_recording_flow():
    ...
    for _ in range(5):
        recorder.record_frame(mock_sim_state.pos, mock_sim_state.vel, mock_sim_state.temp, mock_robot.get_qpos(), torch.tensor(1.0), torch.tensor(2.0), torch.rand(4))
        
    print("[Test] Simulating Cut/Loss (N=80)...")
    mock_sim_state.n_particles = 80
    recorder.mark_strike_start()
    for _ in range(5):
        recorder.record_frame(mock_sim_state.pos, mock_sim_state.vel, mock_sim_state.temp, mock_robot.get_qpos(), torch.tensor(1.5), torch.tensor(2.5), torch.rand(4))
    ...
    with h5py.File(file_path, "r") as f:
        ...
        temp = ep0["observations/state/particles/temp_values"][:]
        ...
        assert temp.shape == (900,), f"Expected 900 temperature values, got {temp.shape[0]}"
        ...
        temp1 = ep1["observations/state/particles/temp_values"][:]
